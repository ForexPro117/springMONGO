...
public class TestController {
    ...
    public String getGeometryBatch() {
        ...
        int chunks = (int) Math.ceil(size / 1d);
        for (int i = 0; i < chunks; i++) {
            Instant start = Instant.now();
            var page = pgeometryRepository.findAllByCreateDateBefore(dateToConvert, PageRequest.of(i, 1)).get();

            StringBuilder builder = new StringBuilder();
            page.forEach(el -> {
                builder.append("INSERT INTO geometry_data (uuid, hash_code, indices, vertices, normals, colors_quantized, colors) VALUES ");
                builder.append(String.format("(%s, '%s', textAsBlob('%s'),  textAsBlob('%s'),  textAsBlob('%s'),  textAsBlob('%s'),  textAsBlob('%s'));",
                        el.getUuid(), el.getHashCode(), Arrays.toString(getByteArray(el.getIndices())), Arrays.toString(getByteArray(el.getVertices())),
                        Arrays.toString(getByteArray(el.getNormals())), Arrays.toString(getByteArray(el.getColorsQuantized())), Arrays.toString(getByteArray(el.getColors()))));
            });
            cassandraTemplate.getCqlOperations().execute("BEGIN BATCH " + builder + " APPLY BATCH;");
